<template>
  <div>
    <h1 class="text-2xl font-bold text-gray-900 mb-6">
      <font-awesome-icon icon="fa-solid fa-users" class="text-indigo-600 mr-2" />
      角色管理器
    </h1>

    <div v-if="characterStore.error" class="bg-red-50 border border-red-200 text-red-700 rounded-lg p-4 mb-4">
    ...
    <template v-if="selectedWorldId">
      <CharacterForm ref="formRef" :worldbuilding-id="selectedWorldId" />
      <CharacterList :worldbuilding-id="selectedWorldId" @edit-character="onEditCharacter" />
    </template>
...
function onWorldChange() {
  if (selectedWorldId.value) {
    characterStore.fetchAll(selectedWorldId.value)
  } else {
